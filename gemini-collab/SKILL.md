...
mkdir -p /c/Users/<user>/work/agy_job
#    write /c/Users/<user>/work/agy_job/TASK.md with the full instructions + URLs + recipe.
...
echo "Read TASK.md in the current directory and carry out every step autonomously, end to end. Report what you did." \
  | python "${CLAUDE_SKILL_DIR}/scripts/agy_pty.py" --idle 900 --timeout 3600 --cwd /c/Users/<user>/work/agy_job
```
...
- Antigravity in skip-permissions mode WILL autonomously execute gmail_sender.py and send emails without user approval if it decides to. This happened in production with the prior gemini-cli and caused two unauthorized midnight emails to an external recipient.
