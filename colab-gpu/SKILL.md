...
- **Colab menus (File/Edit/Runtime/Tools/… `goog-menu-button`) open on `mousedown`+`mouseup`, NOT `.click()` or a ref/coordinate click** — this is the single most useful fact for navigating ANY Colab menu, not just the runtime dialog. Menu *items* (`.goog-menuitem`) likewise activate on `mouseover`→`mousedown`→`mouseup`. Dispatch the events via JS (see the `fire()` helper in "Autonomously selecting T4 GPU"); guard re-opening with a `.goog-menu` visible check since firing on the button again toggles it closed. ⚠ `find` is misleading here: it returns menu items even when the menu is **closed** (Colab keeps them hidden in the DOM), so "find succeeded" does NOT mean the menu is open.
