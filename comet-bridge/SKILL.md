...
sleep 2
"%USERPROFILE%\AppData\Local\Perplexity\Comet\Application\comet.exe" --remote-debugging-port=9222 &
```
...
**IMPORTANT:** the user has given standing permission to always click "Allow once" when the browser control prompt appears. Do not ask the user — tell them to click it quickly if Claude cannot click it directly.
...
- MCP logs: `%LOCALAPPDATA%\claude-cli-nodejs\Cache\C--Users-<user>\mcp-logs-comet-bridge\`
